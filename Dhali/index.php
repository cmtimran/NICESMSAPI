...
<body>
    ...
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        if ($fldBooleans === 1) {
$msg = "Greetings from Dhali’s Amber Nivaas.
...
Advance Payment Received: " . $fldAdvance . "/- tk";
        ...
        $url = "https://labapi.smartlabsms.com/smsapi?user=dhaliresort&password=Dhaka@2021&sender=DhaliResort&msisdn=" . $fldPhnNumber . "&smstext=". $newmsg;
        // $url = "https://api.mobireach.com.bd/SendTextMessage?Username=dhaliresort&Password=Dhaka@2021&From=DhaliResort&To=" . $fldPhnNumber . "&Message=" . $newmsg;
        ...
        if ($conn2) {
            $sql2 = "UPDATE tblBookingRSMS SET fldBooleans = 0,fldSMSCount='$SMSCount', fldMessageId='$MessageId', fldDeliveryStatus='$StatusText', fldSentTime='$fldSentTime' WHERE  PropertyID in ('SM003') and ItemID = " . $ItemID;
            $stmt2 = sqlsrv_query($conn2, $sql2);
            $sql3 = "UPDATE tblCurrentBalance1 SET CurrentCredit = '$CurrentCredit' where PropertyCode = 'SM003'";
    ...
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        if ($fldBooleans === 1) {
            // Checkin
$msg = "Dear $fldGuestName,
Thank you for choosing Dhali’s Amber Nivaas and we hope you have a wonderful stay!";
        } else {
            // CheckOut
$msg = "Dear $fldGuestName,
Thank you for choosing Dhali’s Amber Nivaas and we look forward to welcoming you back soon!";
        }
        ...
        if ($conn2) {
            $sql2 = "UPDATE tblCheckinOut SET fldBooleans = 0, fldSMSCount='$SMSCount', fldMessageId='$MessageId', fldDeliveryStatus='$StatusText', fldSentTime='$fldSentTime' WHERE  PropertyID in ('SM003') and ItemID = " . $ItemID;
            $stmt2 = sqlsrv_query($conn2, $sql2);
            $sql3 = "UPDATE tblCurrentBalance1 SET CurrentCredit = '$CurrentCredit' where PropertyCode = 'SM003'";
            $stmt3 = sqlsrv_query($conn2, $sql3);
            if ($stmt2 === false && $stmt3 === false) {
    ...
    while ($daterow = sqlsrv_fetch_array($datestmt, SQLSRV_FETCH_ASSOC)) {
        ...
        $regsql = "SELECT *  FROM tblCheckinOut WHERE PropertyID in ('SM003') and fldCurrentCredit=0";
        $regstmt = sqlsrv_query($conn, $regsql);

        $findbtsql = "SELECT *  FROM tblCheckinOut WHERE PropertyID in ('SM003') and fldCurrentCredit=1";
...
$msg = "Dear $fldGuestName,
Dhali’s Amber Nivaas wishes you a very Happy Birthday! May your day be filled with unforgettable moments.";
                ...
                if ($conn2) {
                    $regsql2 = "UPDATE tblCheckinOut SET fldSMSCount='$SMSCount', fldMessageId='$MessageId', fldDeliveryStatus='$StatusText',fldCurrentCredit=1, fldSentTime='$fldSentTime' WHERE  PropertyID in ('SM003') and ItemID = " . $RegItemID;
        ...
        while ($findbtrow = sqlsrv_fetch_array($findbtstmt, SQLSRV_FETCH_ASSOC)) {
            ...
            if ($SystemDate != $findbtfldBirtDate) {
                $sql5 = "UPDATE tblCheckinOut SET  fldCurrentCredit=0 WHERE  PropertyID in ('SM003') and ItemID = " . $findbtItemID;
    ...
    //Current Balance
    $ccreditsql = "SELECT * FROM tblCurrentBalance1 where PropertyCode = 'SM003'";
                                        ...
                                        <table id="datatable-buttons" style="overflow:hidden; width: 100% !important;" class="table table-striped table-bordered dt-responsive dataTable no-footer dtr-inline">
                                            ...
                                            <tbody>
                                                ...
                                                if (isset($_POST['submit'])) {
                                                    ...
                                                    $sql = "SELECT ItemID, [Date], fldTime, PropertyID, PropertyName, fldBookingNo, fldArrDate, fldDeptDate, fldNoOfNight, fldNoOfRoom, fldRoomNo, fldRoomType, fldGuestName, fldCompanyName, fldPhnNumber, fldEmail, fldPAX, fldAdvance, fldBooleans, fldSentTime, fldMessageID, fldDeliveryStatus, fldSMSCount, fldCurrentCredit FROM tblBookingRSMS WHERE PropertyID in ('SM003') and  (fldSentTime BETWEEN '$start_date' AND '$end_date') ORDER BY fldSentTime desc";
                                                    $stmt = sqlsrv_query($conn, $sql);
